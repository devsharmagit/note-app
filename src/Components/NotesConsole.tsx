...
const q = query(collection(db, "notes"), where("belongTo", "==", note.noteId))
...
<ArrowBackIosNewRoundedIcon />
        ...
        <div className={`w-60 rounded-lg bg-slate-50 after:bg-black  after:w-1/2 after:h-1 after:block after:rounded-md after:transition-transform ${!personalOpen ? 'after:translate-x-full ' : 'after:translate-x-0'}`}>
          <button name='personal' onClick={(e)=> handleToggleClick(e.currentTarget.name)} className={`inline-flex gap-1 transition-all items-center w-1/2 px-3 py-2 bg-slate-50 ${personalOpen && "!bg-slate-200"} `}>
            <PersonRoundedIcon sx={{ fontSize: 30 }} />
            <span className='font-medium' >
           Personal
            </span>
          </button>
          <button name='shared' onClick={(e)=> handleToggleClick(e.currentTarget.name)} className={`inline-flex gap-1 transition-all items-center w-1/2 px-3 py-2 bg-slate-50 ${!personalOpen && "!bg-slate-200"} `}>
      ...
      <div className={`w-[200%] no-scrolll overflow-scroll h-full flex relative ${personalOpen ? 'left-0' : '-left-full'} transition-all`}>
...

        {
          ...
          <div className='flex items-center justify-between'>
            <span className='text-2xl font-semibold text-gray-700'> # {note.noteCollName} </span> <IconButton onClick={handleDelete}> <DeleteRoundedIcon /> </IconButton>
          </div>
...
<span className='text-xl font-semibold text-gray-700'> Home </span> <hr className='ml-3 border-0 rounded-full bg-gray-300 flex-1 h-[1px]' />
  </div>
  <CollDisplay />
  <hr className='mt-3 border-0 rounded-full bg-gray-300 flex-1 h-[1px]'  />
  <NoteList type="personal" isHome={true}/>
        ...
        <div className='no-scrolll w-1/2 bg-white p-5'>
          { shareCollection ? <ShareNoteDisplay /> : <>
